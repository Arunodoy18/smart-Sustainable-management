...
class SegregationAgent(BaseAgent[WasteClassificationInput, WasteClassificationOutput]):
    ...
    async def process(
        self, input_data: WasteClassificationInput
    ) -> AgentResponse[WasteClassificationOutput]:
        self.log_activity(
            f"Analyzing segregation requirements for user {input_data.user_id}"
        )
        ...
        rules = self.segregation_rules.get(
            category,
            {
                "recyclable": False,
                "special_handling": True,
        ...
        # Update reasoning
        segregation_reasoning = (
            f"{classification_response.reasoning} | Segregation: {rules['notes']}"
        )

        if not classification_response.data.is_segregation_correct:
            segregation_reasoning += (
                f" | WARNING: {classification_response.data.violation_details}"
            )
