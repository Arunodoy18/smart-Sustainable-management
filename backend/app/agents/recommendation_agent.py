...
class RecommendationAgent(BaseAgent[RecommendationInput, RecommendationOutput]):
    ...
    async def process(
        ...
        self.log_activity(
            f"Generating recommendation for {input_data.waste_category} (confidence: {input_data.confidence})"
        )
        ...
        category_templates = self.action_templates.get(
            input_data.waste_category, self.action_templates["plastic"]
        )
