...
from app.schemas.waste import (
    PickupVerificationInput,
    PickupVerificationOutput,
    WasteClassificationInput,
    WasteType,
)
import random
...
class CollectionAgent(BaseAgent[PickupVerificationInput, PickupVerificationOutput]):
    ...
    async def process(
        self, input_data: PickupVerificationInput
    ) -> AgentResponse[PickupVerificationOutput]:
        self.log_activity(
            f"Verifying pickup {input_data.pickup_id} by driver {input_data.driver_id}"
        )
        ...
        if not classifier_response.success:
            return AgentResponse(
                success=False, error="Failed to classify waste during verification"
            )
        ...
        if is_verified:
            ...
            self.log_activity(
                f"Violation: Expected {input_data.waste_type_expected}, got {actual_type}",
                level="WARNING",
            )
        ...
        reasoning = (
            "Pickup matches expected waste type."
            if is_verified
            else "Waste type mismatch detected."
        )
